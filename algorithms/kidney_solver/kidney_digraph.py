...
class Digraph:
    ...
    def __init__(self, n):
        ...
        print("n is: ")
        print(n)
        print("Max n is " + str(max(n)))
        self.vs = [None for i in range(max(n) + 1)]
        for i in n:
            self.vs[i] = Vertex(i)
        self.adj_mat = [[None for x in range(max(n) + 1)] for x in range(max(n) + 1)]
        self.es = []
...
def read_digraph(lines, vertices):
